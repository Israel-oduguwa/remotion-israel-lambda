...
const ActiveCaption: React.FC<{
  ...
  const variantIndex = seededIndex(
    `${sceneId}-exchat-caption-${captionIndex}`,
...
export const ExchatScene: React.FC<ExchatSceneProps> = ({
  entry,
  props,
  sceneIndex,
}) => {
  ...
  const variantIndex = seededIndex(
    `${props.adId}-exchat-media-${sceneIndex}`,
